#!/usr/bin/env bash
# Helper para empaquetar PortWatch con PyInstaller usando uv.
# Uso:
#   ./build_app.sh            # autodetecta macOS/Linux
#   ./build_app.sh mac        # fuerza macOS
#   ./build_app.sh linux      # fuerza Linux
...
if ! command -v uv >/dev/null 2>&1; then
  echo "Falta 'uv'. Instálalo primero: https://github.com/astral-sh/uv" >&2
  exit 1
fi
...
# Asegura dependencias (usa uv para instalar pip pkgs listados)
...
if [[ "${PLATFORM}" == "mac" ]]; then
  ...
  uv run pyinstaller backend/ui/tray_app.py \
    --name PortWatch \
    --windowed \
